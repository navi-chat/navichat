...
const PlaygroundBotInterface = ({ bot_id }: { bot_id: string }) => {
    const BotId = bot_id
    ...
    useEffect(() => {
        const initialCall = async () => {
            ...
            if (res.data) {
                setBot(res.data)
                const companyRes = await getCompanyById({ id: res.data.company })
                const systemPrompt = `You are ${res.data.name}, a support assistant for ${companyRes.data.name}, deployed on ${res.data.website}. Respond only to user queries related to support. If greeted, greet back politely. Do not give unrelated info or list everything. Bot ID: ${res.data.id}. Keep the answer as short as possible and use simple, conversational language. To show an image in the chat, use this format: [Image:https://xyz.com].`;
                setMessages([{ role: "system", content: systemPrompt }, { role: "ai", content: "How can I help you ?" }])
            }
        }
        initialCall()
    }, [BotId])
    ...
    const handleSendMessage = async () => {
        ...
        const newMessages: Message[] = [...messages, { role: "human", content: currentMessage }]
...
export default PlaygroundBotInterface